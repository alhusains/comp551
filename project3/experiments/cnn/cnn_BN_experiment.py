...
# Create a unit defining the convolutional layer, which consists of a convolution operation followed by ReLU.
...
#define a second network of 2 covolutional layer with max pool in between the, followed by 3 fully connected layers.
class Net2(nn.Module):
    def __init__(self):
        super(Net2, self).__init__()
        self.unit1 = Unit2(in_channels=3, out_channels=6)
        self.pool = nn.MaxPool2d(2, 2)
        self.relu = nn.ReLU()
        self.unit2 = Unit2(in_channels=6, out_channels=16)
        self.fc1 = nn.Linear(16 * 5 * 5, 120)
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, 10)
...
for epoch in range(num_epochs):
  ...
  print("For CNN2: Epoch {}, Train Accuracy: {} , TrainLoss: {} , Test Accuracy: {}".format(epoch, train_acc2, train_loss2,
                                                                                        test_acc2))
plt.figure(1)
plt.plot(loss_arr, 'r', label='Loss without BN')
plt.plot(loss2_arr, 'g', label='Loss with BN')
plt.xlabel("Epochs")
plt.ylabel("Loss")
...
plt.figure(2)
plt.plot(train_acc_arr, 'r--', label='Train accuracy of CNN without BN')
plt.plot(train2_acc_arr, 'g--', label='Train accuracy of CNN with BN')
plt.plot(test_acc_arr, 'y', label='Test accuracy of CNN without BN')
plt.plot(test2_acc_arr, 'b', label='Test accuracy of CNN with BN')
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.title('# of Batches: 32')
plt.legend()
...
plt.figure(3)
plt.plot(test_acc_arr, 'r', label='Test accuracy of CNN without BN')
plt.plot(test2_acc_arr, 'g', label='Test accuracy of CNN with BN')
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.title('# of Batches: 32')
plt.legend()
