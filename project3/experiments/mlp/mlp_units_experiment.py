...
def more_units():
    ...
    model = DNN([3072, 1000, 100, 10])  # initialize the model
...
#the accuracy array
acc_array2 = less_units()

plt.figure(1)
plt.plot(acc_array, 'r', label='Test accuracy of MLP with more units')
plt.plot(acc_array2, 'g', label='Test accuracy of MLP with less units')
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.legend()
plt.show()